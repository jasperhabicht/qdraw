...
\def\hawkdrawfileversion{0.8.0}
\def\hawkdrawfiledate{1 September 2026}
...
The current PDF specification supports applying a shading as pattern to a path or filling a path with a shading directly. Using the first approach, it is possible to apply a shading to a path as fill or as stroke. But since the geometry of the shading is independent of that of the path, this requires two compilation runs as TeX needs to store the position of the current drawing in order to be able to position the shading correctly. Therefore, the \macro{hawkdraw} package makes use of the second approach which, however, means that shaidngs cannot applied as strokes to paths. 
