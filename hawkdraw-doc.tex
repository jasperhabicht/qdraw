...
Note that patterns in general inherit the style set to the original path. Therefore, if the option \macro{corner arc} is set to a non-zero value for the original path, the pattern will also be drawn with rounded corners. If this is not desired, the option \macro{corner arc} can be set to \qty{0}{\point} using the option \macro{pattern style set} or \macro{pattern style add}.
